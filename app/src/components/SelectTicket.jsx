...
const SelectTicket = () => {
  ...
  const handleImageUpload = async (e) => {
    ...
    formData.append('api_key', '181788834982891');
    formData.append('upload_preset', 'ml_default');

    try {
      const response = await fetch(
        `https://api.cloudinary.com/v1_1/deiwbxqpu/image/upload`,
        {
          method: 'POST',
          body: formData,
        }
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="w-full h-max-content bg-[#08252B] border border-[#0E464F] rounded-[2rem] p-[1.5rem]">
          <div className="mb-[2rem] flex flex-col gap-4">
            ...
            <div
              className="h-[200px] w-full border-2 border-dashed border-[#0e464f] rounded-xl flex flex-col items-center justify-center cursor-pointer relative overflow-hidden"
            ...
            >
              {imageUrl ? (
                ...
                <div className="text-center">
                  ...
                  />
                  {uploading ? (
                    <div className="text-[#24A0B5]">Uploading...</div>
                  ) : (
                    <>
                      <p className="text-[#24A0B5]">Click to upload photo</p>
                      <p className="text-sm text-[#D9D9D9] mt-2">PNG, JPG (max. 5MB)</p>
                    </>
                  )}
                </div>
              )}
          ...
          <div className="flex flex-row gap-[1.5rem] w-full">
            <button
              type="button"
              className='w-full h-[3.25rem] hover:bg-[#24A0B5]/20 justify-center rounded-xl border border-[#24A0B5] text-[#24A0B5] text-[1rem] text-center transition-all duration-300 disabled:bg-transparent'
            >
              Back
            </button>
            <button
